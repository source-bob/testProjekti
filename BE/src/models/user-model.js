...
const selectUserByNameAndPassword = async (username, password) => {
  try {
    const [rows] = await promisePool.query(
      'SELECT user_id, username, email, registered_at, user_level FROM Users WHERE username=? AND password=?',
      [username, password],
    );
    console.log(rows);
    return rows[0];
...
const selectUserByUsername = async (username) => {
    try {
      const [rows] = await promisePool.query(
        'SELECT user_id, username, password, email, registered_at, user_level FROM Users WHERE username=?',
        [username],
      );
      console.log(rows);
      return rows[0];
...
const editUser = async (id, username, pass, email, user_level) => {
    const sql = `UPDATE users
                SET password = '${pass}', username = '${username}', email = '${email}', user_level = '${user_level}'
                WHERE user_id = ${id}`;
    try {
